...
/*   Updated: 2022/02/19 18:25:13 by lpaulo-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cast_parts(float n, unsigned long long *whole, float *decimal)
{
	*whole = (long long)n;
	*decimal = (n - *whole);
	if (*decimal < 0)
...
static void	print_whole(float n, unsigned long long whole)
{
	if (n < 0.0)
		ft_putchar('-');
	ft_putnbr_ull(whole);
